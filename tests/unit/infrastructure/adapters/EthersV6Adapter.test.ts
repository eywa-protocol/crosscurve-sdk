...
import { EthersV6Adapter } from '../../../../src/infrastructure/adapters/EthersV6Adapter.js';
import type { TransactionRequest } from '../../../../src/types/signer.js';

describe('EthersV6Adapter', () => {
  ...
  describe('getAddress', () => {
    it('should return address from signer', async () => {
      ...
      const adapter = new EthersV6Adapter(mockSigner);
  ...
  describe('signTypedData', () => {
    ...
    it('should call signer.signTypedData with correct parameters', async () => {
      const mockSigner = createMockSigner();
      mockSigner.signTypedData.mockResolvedValue(testSignature);
      const adapter = new EthersV6Adapter(mockSigner);
    ...
    it('should propagate errors from signer.signTypedData', async () => {
      const mockSigner = createMockSigner();
      mockSigner.signTypedData.mockRejectedValue(new Error('User rejected typed data signing'));
      const adapter = new EthersV6Adapter(mockSigner);
    ...
    it('should handle minimal domain', async () => {
      ...
      expect(mockSigner.signTypedData).toHaveBeenCalledWith(minimalDomain, types, value);
    });
  });
